...
export function produceTOPWN (rawRequestClientSeed?: string, rawHashedSecret?: string, rawRequestNonce?: number): TrustfulOrderPayloadWithNonce {
  try {
    return {
      clientSeed:
        rawRequestClientSeed?.length > 0
    ...
    throw new Error('Client seed or secret hash were missformated, please try again.')
  }
}
...
export class ProvablyFairResolver {
   ...
   */
  public static asHexString (clientSeedAsHexString: string, revealedSecretAsHexString: string, contextNonce?: number) : string {
